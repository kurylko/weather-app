export function getAirQualityDescription({co, no2, o3, pm2_5, pm10, so2}) {
    ...
    const isDataAvailable = [co, no2, o3, pm2_5, pm10, so2 ].every(param => param !== null && param !== undefined);
    if (!isDataAvailable) {
    ...
    const descriptions = [pm25Description, pm10Description, no2Description, o3Description, so2Description, coDescription];

    const overallQuality = descriptions.includes('very bad') ? 'very bad'
        : descriptions.includes('bad') ? 'bad'
            : descriptions.includes('mediocre') ? 'mediocre'
                : 'good';
    ...
    return (
        {overallAirDescription, detailedAirDescription}
    );
}